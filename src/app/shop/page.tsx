...
import { getShopContent } from "@/lib/data-helper"
import { Search, Grid3x3, List, Star } from "lucide-react"
import Link from "next/link"

export default function ShopPage() {
  const products = getProducts()
  const content = getShopContent()
    ...
    <div className="flex min-h-screen flex-col bg-[#141618]">
      ...
      <main className="flex-1">
        <div className="border-b border-[#33383D] bg-[#1A1C1F] py-6 text-center">
          <h1 className="text-3xl font-bold uppercase text-white" style={{ fontFamily: "var(--font-lato)" }}>{content.title}</h1>
        </div>
          ...
          <div className="flex flex-col gap-6 lg:flex-row">
            <div className="flex-1 lg:w-[70%]">
              <div className="mb-4 flex items-center justify-between border border-[#33383D] bg-[#1A1C1F] px-4 py-2">
                ...
                <p className="text-xs text-[#9B9C9E]">{content.resultsText.replace("{count}", String(products.length))}</p>
                <select className="border border-[#33383D] bg-[#141618] px-3 py-1 text-xs text-[#9B9C9E] focus:outline-none">{content.sorting.map((s: string) => <option key={s}>{s}</option>)}</select>
              </div>
            ...
            <aside className="lg:w-[28%]">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-1">
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.cartTitle}</h3><p className="text-xs text-[#9B9C9E]">{content.sidebar.cartEmpty}</p></div>
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.priceTitle}</h3><div className="mb-3"><div className="relative h-1 rounded-full bg-[#33383D]"><div className="absolute h-1 rounded-full bg-[#E21818]" style={{ left: "10%", right: "20%" }}></div><div className="absolute h-3 w-3 rounded-full bg-[#E21818] -top-1" style={{ left: "10%" }}></div><div className="absolute h-3 w-3 rounded-full bg-[#E21818] -top-1" style={{ right: "20%" }}></div></div></div><p className="mb-3 text-xs text-[#9B9C9E]">{content.sidebar.priceRange}</p><button className="w-full bg-[#E21818] py-2 text-xs font-bold uppercase text-white hover:bg-[#C51515]">{content.sidebar.priceButton}</button></div>
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.searchTitle}</h3><div className="flex"><input type="text" placeholder={content.sidebar.searchPlaceholder} className="flex-1 border border-[#33383D] bg-[#141618] px-3 py-2 text-xs text-white placeholder:text-[#9B9C9E] focus:outline-none" /><button className="bg-[#E21818] px-3 py-2 text-white hover:bg-[#C51515]" aria-label="Search"><Search className="h-4 w-4" /></button></div></div>
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.arrivalsTitle}</h3><ul className="space-y-3">{newArrivals.map(p => <li key={p.slug}><Link href={`/product/${p.slug}`} className="flex items-center gap-3 group"><img src={p.img} alt="" className="h-14 w-14 object-cover" /><div><p className="line-clamp-2 text-xs text-[#9B9C9E] group-hover:text-[#E21818]">{p.name}</p><p className="text-xs font-bold text-[#E21818]">{p.price}</p></div></Link></li>)}</ul></div>
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.featuredTitle}</h3><ul className="space-y-3">{featuredProducts.map(p => <li key={p.slug}><Link href={`/product/${p.slug}`} className="flex items-center gap-3 group"><img src={p.img} alt="" className="h-14 w-14 object-cover" /><div><p className="line-clamp-2 text-xs text-[#9B9C9E] group-hover:text-[#E21818]">{p.name}</p><div className="flex items-center gap-1"><p className="text-xs font-bold text-[#E21818]">{p.price}</p><div className="ml-1 flex">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-2.5 w-2.5 fill-[#E21818] text-[#E21818]" />)}</div></div></div></Link></li>)}</ul></div>
                <div className="bg-[#1A1C1F] p-4"><h3 className="mb-3 border-b border-[#33383D] pb-2 text-sm font-bold uppercase text-white">{content.sidebar.tagsTitle}</h3><div className="flex flex-wrap gap-2">{content.tags.map((tag: string) => <span key={tag} className="border border-[#33383D] px-2 py-1 text-[10px] uppercase text-[#9B9C9E] hover:border-[#E21818] hover:text-[#E21818] cursor-pointer">{tag}</span>)}</div></div>
              </div>
            </aside>
          </div>
        </div>
